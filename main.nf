...
process runArtic {
    ...
    run_artic.sh \
        ...
        ${basecall_model} ${params._scheme_name} ${scheme_dir} \
...
process nextclade {
    ...
    script:
    def tag_str = nextclade_data_tag ?: "latest"
    """
    nextclade --version | sed 's/ /,/' > nextclade.version
    echo "nextclade_data_tag,$tag_str" >> nextclade.version
...
process getPangolinData {
    label "pangolin"
    cpus 1
    output:
        path "pangolin_data", emit: data
    script:
    """
    mkdir -p pangolin_data
    if command -v git &> /dev/null; then
        git clone --depth 1 https://github.com/cov-lineages/pangolin-data.git pangolin_data
    else
        curl -fsSL https://github.com/cov-lineages/pangolin-data/archive/refs/heads/master.tar.gz | tar -xz --strip-components=1 -C pangolin_data
    fi
    """
}
...
process pangolin {
    ...
    script:
    def datadir_opt = (pangolin_data.name != 'OPTIONAL_FILE' && pangolin_data.name != 'NO_DATA') ? "--datadir ${pangolin_data}" : ""
    """
    # set cache for snakemake to prevent permission issues
    export XDG_CACHE_HOME=\$(pwd -P)

    pangolin --all-versions 2>&1 | sed 's/: /,/' > pangolin.version
    pangolin --threads ${task.cpus} $params._pangolin_options $datadir_opt consensus.fasta
    """
}
    ...
    main:
        ...
        if ((samples.getClass() == String) && (samples.startsWith("Error"))){
            ...
            if (params.update_data) {
                nextclade_data = getNextcladeData(params.nextclade_data_tag ?: '')
                clades = nextclade(
                    all_consensus[0],
                    nextclade_data.data.collect(),
                    params.nextclade_data_tag ?: 'latest')
            } else {
                clades = nextclade(
                    all_consensus[0],
                    file("${nextclade_dataset}/${nextclade_data_tag}", type: 'dir'),
                    nextclade_data_tag)
            }
            // pangolin
            if (params.update_data) {
                pangolin_data = getPangolinData()
            } else {
                pangolin_data = Channel.fromPath("$projectDir/data/OPTIONAL_FILE")
            }
            pangolin(all_consensus[0], pangolin_data.collect())
...
workflow {
    ...
    // For nextclade choose the most recent data from the nextclade_data git submodule, or if nextclade_data_tag is set in params use that
    // if the user specifies --nextclade_data_tag and --update_data - that tag will be pulled a fresh and used
